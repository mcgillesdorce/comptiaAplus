        ...
        <div className="grid grid-cols-2 gap-3">
          {TOOLS.filter((t) => !t.highlight).map(({ href, icon: Icon, label, sublabel }) => (
            <Link
              key={href}
              href={href}
              className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-4 transition-all active:scale-[0.99] hover:border-brand-300"
            >
